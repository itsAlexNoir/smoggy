...
def request_climate_info(init_date, end_date, estacion='Retiro'):
    ...
    url = os.path.join(preurl, init_date.strftime('%Y-%m-%dT%H:%M:%SUTC'), 'fechafin',
                       end_date.strftime('%Y-%m-%dT%H:%M:%SUTC'), 'estacion', estaciones_meteo[estacion])
    # Get AEMET API key
    with open('aemet-api-key', 'r') as f:
        key = f.readlines()[0]

    querystring = {"api_key": key}
    print('Requesting climate info between {} and {} for station {}'.format(init_date, end_date, estacion))
    response = requests.get(url, params=querystring)
    if response.json()['estado']!=200:
        print('Climate request has an error: {}'.format(response.json()['descripcion']))

    clima_info = requests.get(response.json()['datos'])
    return clima_info.json()
